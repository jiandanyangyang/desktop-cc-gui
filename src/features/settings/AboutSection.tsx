...
const SOCIAL_CHIP_CLASSES =
  "flex size-9 cursor-pointer items-center justify-center rounded-xl border border-border-button-default bg-background-secondary-default text-foreground-icon-secondary transition-colors hover:bg-background-secondary-hover hover:text-foreground-icon-primary";

/** Round brand-icon chip that opens `url` in the system browser. */
function SocialLink({ url, label, path }: { url: string; label: string; path: string }) {
  return (
    <button
      type="button"
...
function DouyinChip() {
  ...
  return (
    <div className="group relative">
      ...
      >
        {copied ? (
          <Check className="size-[18px] text-notification-success-foreground" aria-hidden />
        ) : (
      ...
      <div
        id="douyin-popover"
        role="tooltip"
        className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 flex-col items-center gap-2 rounded-2xl border border-separator-border bg-background-primary-default p-3 shadow-xl group-hover:flex group-focus-within:flex"
      >
        <img
          src={douyinImage}
        ...
        />
        <p className="text-caption-1-regular whitespace-nowrap text-text-secondary">
          {t("settings.douyinId")}
        </p>
      </div>
    </div>
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        <div className="flex w-full flex-col items-center gap-3 py-2">
          ...
          <p className="text-body-2-regular text-text-secondary">{t("settings.communityQrTip")}</p>
        </div>
      </div>
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2 px-3 py-1">
          ...
          <SocialLink
            url={XIAOHONGSHU_URL}
            label={t("settings.socialXiaohongshu")}
            path={XIAOHONGSHU_PATH}
          />
          <DouyinChip />
          <SocialLink url={YOUTUBE_URL} label={t("settings.socialYoutube")} path={YOUTUBE_PATH} />
        </div>
      </div>
